...
class WebController extends Controller
{
    public function home(){
        ...
        // }else{
            $tools          = Tool::whereNull('tool_id')->where('status', 1)->where('slug', '!=','ti-84-calculator')->orderByRaw("FIELD(title, 'Ti 84 calculator', 'Ti-30xs Calculator Online', 'online graphing calculator')")->get();
            $homeSetting    = Setting::where('type', 'home')->first();
            seo()
            ->title($homeSetting->meta_title)
            ->description($homeSetting->meta_description)
        ...
        return view('web.home', compact('homeSetting','tools'));
    }
