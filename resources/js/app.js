...
window.initTabs = function () {
    const tabButtons = document.querySelectorAll(".tabs[target-data]");
    ...
    tabButtons.forEach(button => {
        button.addEventListener("click", () => {
            const targetId = button.getAttribute("target-data");
            ...
            // sab contents ko hidden + grid remove
            tabContents.forEach(c => {
                c.classList.add("hidden");
                c.classList.remove("grid");
            });

            // sirf target content ko show karo
