...
@section('content')
    <form action="{{ route('admin.settings.store') }}" method="POST" enctype="multipart/form-data" class="pr-3">
        ...
        <div class="bg-white rounded-2xl grid grid-cols-1 gap-4 p-4 mb-4">
            <h2 class="text-lg ps-1 font-bold text-gray-900">Home Settings</h2>
        </div>
        <div class="grid grid-cols-12 gap-4">
            <div class="bg-white rounded-2xl col-span-12 p-4">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" id="tool_textarea" rows="4" class="tool_textarea bg-gray-200 text-sm py-1 border-gray-300 w-full rounded-md px-2 ring-gray-500 focus:ring-sky-600 focus:outline-none focus:border-sky-600">{{ @$settings->data }}</textarea>
                </div>
